...
| `/warden-compress --agent a --rule <id> [--dry-run]` | Proposes a compressed rewrite of a measured rule (one model call; rent is length/4, so half the characters is half the rent) and queues it as a **swap candidate**: the selector measures it against the active set *minus* the original (on top of the identical original its delta would be ~0 by construction). The original is never auto-removed — once the variant is active, the original is redundant and exits via its own re-audits |
...
| `/warden-power [--agent a] [--target-saving N] [--runs N] [--rent N] [--json]` | Zero-token power planner: from the agent's own recorded run-to-run variance, reports the minimum detectable saving at each run count and the runs a target saving needs for 80%/90% power — so a verification burn is provably adequately powered before it spends. Conservative (uniform allocation; the Neyman top-up only tightens) |
